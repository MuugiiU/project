...
const createSupplier = async (req: Request, res: Response, next: NextFunction) => {
    const { supplier_name, supplier_address, supplier_phone, supplier_email } = req.body;

    if (!supplier_name || !supplier_address || !supplier_phone || !supplier_email) {
        res.status(400).json({ message: "Нэр, Имэйл, Утасны дугаар эсвэл Нууц үг байхгүй байна." });
    }
    try {
        ...
        res.status(201).json({ message: "Шинэ түрээслүүлэгч амжилттай бүртгэгдлээ.", supplier});
    } catch (err) {
        res.status(400).json({ message: `Шинэ түрээслүүлэгч бүртгэх гэтэл алдаа.`});
    }
